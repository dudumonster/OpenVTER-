...
class VehicleDetModule:
    ...
    def _nms_rotated(self,pts_cat, scores_cat, cat_np):
        """
        执行旋转非极大值抑制(NMS)的函数，用于过滤重叠的检测框
        参数:
            pts_cat: 连接在一起的点集，用于生成边界框
            scores_cat: 对应的置信度分数
            cat_np: 类别标签数组
        返回:
            过滤后的边界框，通过NMS处理后的结果
        ...
        boxes = torch.from_numpy(np_boxes).to(self.device)
        # labels = torch.from_numpy(np_labels).cuda()  # 这行被注释掉了，可能是将类别标签转换为张量

        # 执行旋转NMS，参数为边界框坐标、分数、阈值和类别
        # boxes[:, :5] - 边界框坐标和角度
        # boxes[:, 5] - 置信度分数
        # 0.3 - NMS阈值
        # boxes[:, 6] - 类别信息
        dets, keep_inds = nms_rotated(boxes[:, :5], boxes[:, 5], 0.3,  boxes[:, 6])
        # print(dets)  # 这行被注释掉了，可能是用于调试输出检测结果
        # 根据NMS保留的索引返回过滤后的边界框
        return boxes[keep_inds]
