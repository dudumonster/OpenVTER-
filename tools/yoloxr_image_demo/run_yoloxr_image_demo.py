...
# -*- coding: utf-8 -*-
"""
Quick image demo for YOLOX-R on a few frames sampled from the demo video.
Outputs annotated frames under tools/yoloxr_image_demo/output/ by default and
does not touch the main pipeline or configs.
"""
import argparse
import json
import os
...
# Ensure repo root on sys.path so detection/... imports work when executed from tools/
...
def load_road_mask(road_config_path: Optional[str], target_shape: Tuple[int, int], use_mask: bool = True) -> Optional[np.ndarray]:
    ...
    # binarize to uint8 for bitwise_and
...
def run_demo(args):
    ...
    for idx, frame_bgr in frames:
        ...
        out_path = os.path.join(args.output_dir, f"{base}_frame{idx:06d}.jpg")
        cv2.imwrite(out_path, drawn)
        print(f"Saved {out_path}")


def parse_args():
    parser = argparse.ArgumentParser(description="YOLOX-R image demo on sampled frames.")
    parser.add_argument(
        "--video_config",
        default="config/demo_config/video_config/20220303_5_E_300_yolox_linux.json",
        help="Path to video config JSON (contains detection/road configs and video path).",
    )
    ...
    parser.add_argument(
        "--output_dir",
        default="tools/yoloxr_image_demo/output",
